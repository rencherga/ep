...
$sql = "select TIMESTAMPDIFF(DAY, NOW(), mdate) FROM tbl_meeting WHERE meetingid = " . $mid;
$diff = getData($sql);
if ($diff > 0)
{
    echo "<div class=\"panel-heading\">Future: " . $meet->m_title . " starts in " . $diff . " days.</div>\n";
}
else
{
    $diff = ltrim($diff,'-');
    echo "<div class=\"panel-heading\">Past: " . $meet->m_title . " ended " . $diff . " days ago.</div>\n";
}
