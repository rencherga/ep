...
echo "    <title>Roadmaps - ProductPlan</title>\n";
...
echo "            <h1>Plans</h1>\n";
echo "            <br>\n";
echo "            <a title=\"Create a new roadmap\" class=\"btn btn-success\"  href=\"/apps/newmp.php\">New Roadmap</a>\n";
...
$sql = "SELECT uid from tbl_test_table order by uid desc";
$uids = getArray($sql);
...
foreach ($uids as $val)
{
    ...
    $u = $val['uid'];
    $sql = "select title from tbl_test_table where uid = " . $u;
    $v_title = getData($sql);
    $sql = "select description from tbl_test_table where uid = " . $u;
    $v_desc = getData($sql);
    $sql = "select owner from tbl_test_table where uid = " . $u;
    $v_owner = getData($sql);
    $sql = "select DATE_FORMAT(ts,'%M %d, %Y') from tbl_test_table where uid = " . $u;
    $v_ts = getData($sql);
    
    echo "\t<TR <tr class='roadmap roadmapable-row share shared shared_object-row' data-editor='true' data-id='" . $u ;
    echo "' data-name='" . $v_title . "' data-snapshot='false' data-type='roadmap' data-viewonly='false'>\n";
    echo "<td class='title'><a onclick=\"dataLayer.push({&#39;event&#39;: &#39;open_shared_roadmap&#39;});\" href=\"" . $u . "\">";
    echo "" . $v_title . "</a>\n</td>\n";
    echo "<td class='description'>\n" . $v_desc . "</td>\n"; 
     echo "<td class='owner'>" . $v_owner . "</td>\n";
    echo "<td class='follow-bell-icon'></td>\n<td class='last_updated_at'>" . $v_ts . "</td>\n";
